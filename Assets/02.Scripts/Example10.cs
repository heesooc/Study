...
public class Example10 : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 정렬
        Debug.Log(string.Format("{0, -5}:{1}", hour, minute)); //1    :15  <- 1 왼쪽정렬
        Debug.Log(string.Format("{0, 5}:{1}", hour, minute)); //    1:15  <- 1 오른쪽정렬
        ...
        // 2-2. 문자열 보간 (사이를 채우다)
        // $"텍스트(변수, 서식)텍스트"
        Debug.Log($"소수점 1자리만 표현: {fValue:F1}"); //쉼표없이 fValue 하는 것이 더 안전하다
