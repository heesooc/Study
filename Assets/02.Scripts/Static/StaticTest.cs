...
public class StaticTest : MonoBehaviour
{
    void Start()
    {
        ...
        //student1.Hello(); (x)
        MyStudent.Hello(); //클래스 자체에서 o 객체로부터x

        //MyStudent.Number = 123; // 맘대로 조정안되게 하고싶어.

        Debug.Log($"나의 학생의 수: {MyStudent.GetNumber()}명");


        // static은 유틸리티 관련 함수를 만들거나 사용할 때 유용하다.
        // 유틸리티 함수란: 게임 개발 중 반복적으로 사용되는 '유용'하고 '실용'적인 함수
        // string(문자열), Mathf(수학), Vector2(벡터), Debug(디버그)
        string.Format("{0}", 1);
        Mathf.Max(1, 2, 3, 43);
